...
// standing in for the real logger Pull/Push otherwise use to decide
// whether to stream a plugin's log file to stdout.
func testLogger() *slog.Logger {
	return slog.New(slog.NewTextHandler(io.Discard, nil))
}

// verboseTestLogger is like testLogger, but with debug-level logging
// enabled, so Pull/Push stream the plugin's log file to stdout.
...
func TestDetect(t *testing.T) {
	tests := []struct {
		...
		{"oci purl", cdx.Component{Type: cdx.ComponentTypeContainer, PackageURL: "pkg:oci/nginx@sha256:abc"}, "oci", false},
		{"docker purl", cdx.Component{Type: cdx.ComponentTypeLibrary, PackageURL: "pkg:docker/nginx@1.27.0"}, "docker", false},
		{"unrelated library", cdx.Component{Type: cdx.ComponentTypeLibrary, PackageURL: "pkg:npm/left-pad@1.3.0"}, "npm", false},
		{"no purl", cdx.Component{Type: cdx.ComponentTypeContainer}, "", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := Detect(tt.component)
			if tt.wantErr {
				if err == nil {
...
func TestPullRemovesLogFileAfterSuccess(t *testing.T) {
	bin := buildFakePlugin(t)
	baseDir := t.TempDir()
...
func TestPullRemovesLogFileAfterFailure(t *testing.T) {
	bin := buildFakePlugin(t)
	baseDir := t.TempDir()
...
func TestPullVerboseStreamsLogToStdout(t *testing.T) {
	bin := buildFakePlugin(t)
	baseDir := t.TempDir()
	...
	if pullErr != nil {
		t.Fatalf("Pull returned error: %v", pullErr)
	}

	want := "[pull pkg:oci/nginx@1.27] "
	if !strings.Contains(buf.String(), want) {
		t.Errorf("stdout = %q, want it to contain the %q prefix identifying which plugin invocation the streamed line is from", buf.String(), want)
	}
	if !strings.Contains(buf.String(), "starting pull") {
		t.Errorf("stdout = %q, want it to contain the streamed plugin log line", buf.String())
	}
}

func TestPullNotVerboseDoesNotStreamLogToStdout(t *testing.T) {
	bin := buildFakePlugin(t)
	baseDir := t.TempDir()
	...
	if buf.Len() != 0 {
		t.Errorf("stdout = %q, want empty (non-verbose must not stream the plugin log)", buf.String())
	}
}
...
func TestPush(t *testing.T) {
	...
	result, err := Push(bin, component, baseDir, "registry.example.com/mirror", testLogger())
	if err != nil {
...
func TestPushWithoutPriorPull(t *testing.T) {
	...
	if _, err := Push(bin, component, t.TempDir(), "registry.example.com/mirror", testLogger()); err == nil {
...
func TestPushFailure(t *testing.T) {
	...
	if _, err := Push(bin, component, baseDir, "registry.example.com/mirror", testLogger()); err == nil {
